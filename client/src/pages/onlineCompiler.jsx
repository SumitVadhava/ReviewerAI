...
//   const [copiedStates, setCopiedStates] = useState({}); // Track copy status for each code block
//   const editorRef = useRef(null);
//   const copyTimeoutRef = useRef({}); // Store timeouts for each code block
...
//       alert('No review data to download. Please analyze your code first.');
//       return;
//     }
...
//             .code-block { background: #4a4a4a; padding: 15px; border-left: 4px solid #333; margin: 15px 0; font-family: monospace; white-space: pre-wrap; color: #d4d4d4; }
...
//     alert('HTML report downloaded. Open in a browser and print/save as PDF.');
//   };
...
//       if (matchedCategory && results[matchedCategory]) {
//         const contentLines = lines.slice(1).filter(line =>
//           line.trim() !== code.trim() // only filter full duplicate code
//         );

//         // Remove trailing empty lines, ---, and # artifacts
//         let sectionContent = contentLines.join('\n').trim();
//         sectionContent = sectionContent
//           .replace(/\n---\s*$/, '') // Remove trailing ---
//           .replace(/\n#+$/, '')     // Remove trailing # (e.g., # or ##)
//           .trim();

//         results[matchedCategory].content = sectionContent;

//         // Extract code blocks only once
...
//   const highlightCode = (code, language = 'java') => {
...
//     return escapedCode
//       .replace(comments, '<span style="color: #808080;">$1</span>')
...
//       alert('Failed to get review from API. Please check the server and try again.');
//     } finally {
...
//       // Handle code blocks
//       if (trimmedLine.startsWith("```") && codeBlockIndex < data.codeBlocks.length) {
//         flushList();
//         const block = data.codeBlocks[codeBlockIndex];
//         const blockKey = `${category}-${codeBlockIndex}`; // Unique key for each code block
//         codeBlockIndex++;
//         if (block) {
//           elements.push(
//             <div key={`code-${i}`} className="my-6">
//               <div className="bg-gray-900 rounded-lg shadow-md border border-gray-700 overflow-hidden">
//                 <div className="bg-gray-800 px-4 py-2 flex items-center justify-between text-sm text-gray-400 border-b border-gray-700">
//                   <span className="font-mono text-green-400">{block.language}</span>
//                   <button
//                     onClick={() => {
//                       navigator.clipboard.writeText(block.code);
//                       setCopiedStates((prev) => ({ ...prev, [blockKey]: true }));
//                       // Clear previous timeout for this block
...
//         // Skip to the closing ```
//         i++;
//         while (i < remainingLines.length && !remainingLines[i].trim().startsWith("```")) i++;
//         continue;
//       }
...
//                     ? "cursor-default text-white" // ✅ Logo gets normal cursor
//                     : "cursor-pointer"
//                   } ${activeTab === tab.id
...
//               <div className="w-12 bg-gray-900 border-r border-gray-800 p-4 text-right text-gray-500 text-sm font-mono leading-6 select-none overflow-hidden">
//                 {lineNumbers}
//               </div>
//               <textarea
//                 ref={editorRef}
...
//                 className="flex-1 bg-transparent p-4 text-gray-100 font-mono text-sm leading-6 resize-none outline-none"
...
//           matchedCategory = 'Improvement'; // Redirect all post-Security content to Improvement
//         }
...
//   //   // Escape HTML characters in the input code to prevent raw HTML display
...
//     // Escape <, >, & but NOT quotes — otherwise span attributes break
//     let escapedCode = code
//       .replace(/&/g, '&amp;')
//       .replace(/</g, '&lt;')
//       .replace(/>/g, '&gt;');
...
//     // Apply syntax highlighting
//     escapedCode = escapedCode
//       .replace(comments, '<span style="color: #7A7777FF;">$1</span>')
//       .replace(strings, '<span style="color: #98c379;">$1</span>')
//       .replace(keywords, '<span style="color: #61afef; font-weight: bold;">$1</span>')
//       .replace(numbers, '<span style="color: #d19a66;">$1</span>')
//       .replace(annotations, '<span style="color: #c678dd;">$&</span>');
...
//         // Pad with empty cells if the row has fewer cells than headers
